...
class GHLClient {
    ...
    // --- Calendars ---
    async createCalendarGroup(locationId, data) {
    ...
    // --- Custom Values ---
    async createCustomValue(locationId, data) {
...
const server = new Server({ name: "ghl-mcp-server", version: "0.4.0" }, { capabilities: { tools: {} } });
    ...
    try {
        switch (name) {
            case "create_french_contact": {
                const { firstName, lastName, email, phone, serviceTag } = args;
            ...
            case "update_user_signature": {
                const { userId, signatureHtml } = args;
