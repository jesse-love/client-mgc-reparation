...
const QuoteWizard: React.FC = () => {
    ...
    const handleTimeSelect = (time: string) => {
        setWizardData(prev => ({ ...prev, appointmentTime: time }));
    };
                    ...
                    <div>
                         ...
                         <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            {wizardData.appointmentDate && (
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{t.contactForm.appointmentTime}</label>
                                    {availableSlots.length > 0 ? (
                                        <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                                            {availableSlots.map(time => (
                                                <button type="button" key={time} onClick={() => handleTimeSelect(time)} className={`px-3 py-2 rounded-md text-sm font-semibold transition-colors duration-200 ${wizardData.appointmentTime === time ? 'bg-orange-500 text-brand-dark shadow' : 'bg-gray-100 text-gray-700 hover:bg-orange-200 dark:bg-slate-700 dark:text-gray-200 dark:hover:bg-orange-500/20'}`}>
                                                    {time}
                                                </button>
                                            ))}
                                        </div>
